...
export const sumItems = (cartItems) => {
  ...
  let estimated = cartItems
    .reduce((total,product) =>  product.delivery *cartItems.length , 0)
    .toFixed(2);
  
  let total = cartItems
    .reduce((total, product) => 
        (product.price * product.quantity + total  ) + product.delivery *(cartItems.length - product.quantity) , 0)
    .toFixed(2);
  ...
  .reduce((total, product) =>    total  + product.price * product.quantity , 0);
    ...
    // let addsubTotal = cartItems
    // .reduce((subTotal, product) =>  subTotal + total, 0)
    // .toFixed(2);


  
    return { itemCount, total, estimated , subTotal};
